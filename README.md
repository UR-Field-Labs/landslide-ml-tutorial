...
2. (Specifically for generating susceptibility map) If you did not download from the Google Drive in the last step, download the *terrain* folder [here (inside the data folder)](https://drive.google.com/drive/folders/1l_BDZlnwOz8s2z7qVb9rNnJSR0NGSLPH?usp=drive_link) and put it in the *data* folder downloaded from the previous step (inside landslide-ml-tutorial). This folder is absent in the Github copy from Step 5 because of its size, so we download it separately.
